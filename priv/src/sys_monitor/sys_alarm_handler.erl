...
%%% @doc
%%      当内存超过预警线 会在此做出处理
%%      Alarm : {resource_limit, memory, node()}
%%------------------------------------------------------------------------------

-module(sys_alarm_handler).

-behaviour(gen_event).

-include("common.hrl").

-export([start_link/0, stop/0, register/2, set_alarm/1,
...
-export([handle_event_memory_set_alarm/1, handle_event_memory_clear_alarm/1]).
...
handle_set_alarm({{resource_limit, Source, Node}, []}, State) ->
  ?T("~s resource limit alarm set on node ~p~n", [Source, Node]),
  {ok, maybe_alert(fun dict:append/3, Node, Source, State)};
handle_set_alarm({file_descriptor_limit, []}, State) ->
  ?T("file descriptor limit alarm set~n"),
  {ok, State};
handle_set_alarm(Alarm, State) ->
  ?T("alarm '~p' set~n", [Alarm]),
  {ok, State}.

handle_clear_alarm({resource_limit, Source, Node}, State) ->
  ?T("~s resource limit alarm cleared on node ~p~n", [Source, Node]),
  {ok, maybe_alert(fun dict_unappend/3, Node, Source, State)};
handle_clear_alarm(file_descriptor_limit, State) ->
  ?T("file descriptor limit alarm cleared~n"),
  {ok, State};
handle_clear_alarm(Alarm, State) ->
  ?T("alarm '~p' cleared~n", [Alarm]),
  {ok, State}.

%% 处理内存报警的回调警报
%% 虚拟机使用内存超过警戒线
handle_event_memory_set_alarm(Alarm) ->
  lib_system_config:close_service(),%内存占用过高 关闭玩家登录入口
  ?T("------------handle_event_memory_set_alarm alarm : ~p  ~n", [Alarm]),
  ?Error(sys_monitor_logger, "handle_event_memory_set_alarm  Alarm : ~p ~n", [Alarm]),
  sys_background_gc:run(),%手动垃圾回收GC
  set_alarm(Alarm),
  ok.

%% 处理内存报警的回调警报
%% 虚拟机使用内存低于警戒线
handle_event_memory_clear_alarm(Alarm) ->
  ?T("------------handle_event_memory_clear_alarm alarm : ~p  ~n", [Alarm]),
  ?Error(sys_monitor_logger, "handle_event_memory_clear_alarm  Alarm : ~p ~n", [Alarm]),
  %GC回收 内存下降 开放玩家登录入口
  lib_system_config:open_service(),
  clear_alarm(Alarm),
  ok.