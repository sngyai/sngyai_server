...
get_acc_id() ->
    case application:get_env(?APP,acc_id) of
        {ok, Acc_Id} -> if Acc_Id > 0 andalso Acc_Id =< 45 -> Acc_Id; true->throw("acc_id id overflow!") end;
        _ -> throw(acc_id)
    end.
%% 获取服唯一ID 区间：服ID(1-9999)
get_server_id() ->
    case application:get_env(?APP,server_id) of
        {ok, Server_Id} -> if Server_Id >= 0 andalso Server_Id =< 9999 -> Server_Id; true->throw("server id overflow!") end;
        _ -> throw(server_id)
    end.
